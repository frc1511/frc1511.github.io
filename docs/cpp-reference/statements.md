...
The `switch` statement solves the same problem as the `if-else-if` statement, but it is easier to read usually more efficient. The `switch` statement evaluates an expression, and if the expression matches one of the cases, the block of code under the case is executed. If the expression does not match any of the cases, the block of code under the `default` case is executed. The `break` statement is used to exit each case in the `switch` statement. If `break` is missing, the program's execution will 'fall through' to the next case, executing its code as well, which is often not the desired behavior.
